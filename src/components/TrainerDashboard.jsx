...
import { getTrainerAuth, saveTrainerAuth } from "../trainerAuth";
...
export default function TrainerDashboard({ onLogout }) {
  ...
  function updateTeaPaymentStatus(orderId, paymentStatus) {
    const next = {
      ...teaOrderMeta,
      [orderId]: {
        ...(teaOrderMeta[orderId] || {}),
        payment_status: paymentStatus,
        updated_at: new Date().toISOString(),
      },
    };
    saveTeaOrderMeta(next);
  }

  function printTeaOrder(order) {
    const paymentStatus = teaOrderMeta[order.id]?.payment_status || (order.payment_method === "upi" ? "paid" : "pending");
    printHtmlDocument(
                      ...
                      <div>
                        ...
                        {order.notes && (
                          <div style={{ marginTop: 12, background: "#f8f6ff", borderRadius: 12, padding: 12, border: "1px solid #e0d7f5" }}>
                            ...
                            <p style={{ margin: 0, color: "#1e1030", fontSize: 13, lineHeight: 1.6 }}>{order.notes}</p>
                          </div>
                        )}
                      </div>
                    </div>
